...
def create_tweet_thread(session, auth, tweets):
    ...
    if response.status_code == 201:
        ...
        for tweet in tweets[1:]:
            ...
            time.sleep(60)  # Add a 60-second delay between tweets in the thread
            ...
            if response.status_code == 201:
                ...
                # Ignore 429 errors and continue without logging
                time.sleep(60)  # Wait for a while before retrying
                continue  # Skip logging the error and just wait
            else:
...
def schedule_tweets(session, auth):
    ...
    logging.info("Scheduler started, will tweet every 2 hours...")
    scheduler.start()
